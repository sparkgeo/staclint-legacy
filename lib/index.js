...
document.addEventListener('DOMContentLoaded', event => {
  ...
  const editorFormLabel = document.querySelector('.form__label-editor');
  const defaultStacVersion = document.querySelector('#defaultStacVersion');
  const editorToggle = document.querySelector('#editorToggle');
  const editorUrlInput = document.querySelector('#stacUrl');
  const externalUrlFormLabel = document.querySelector('.form__label-external');
  ...
  const displayValidationSuccess = () => {
  ...
  const toggleEditor = () => {
    ...
    toggleArrow.innerHTML = toggleArrow.innerHTML === '▸' ? '◂ ' : '▸';
